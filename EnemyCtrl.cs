...
public class EnemyCtrl : MonoBehaviour
{
	...
	void Chasing()
	{
		...
		// 2m以内に近づいたら攻撃
		if (Vector3.Distance(attackTarget.position, transform.position) <= 2.1f)
		{
	...
	void Damage(AttackArea.AttackInfo attackInfo)
	{
		...
		if (GetComponent<NetworkView>().isMine){
			...
			GetComponent<NetworkView>().RPC("DamageMine", GetComponent<NetworkView>().owner, attackInfo.attackPower);
		}
	}
